	...
	async def sauce(self, ctx, image_url=None):
		if image_url := ctx.message.attachments[0].url if ctx.message.attachments else image_url if image_url.startswith(
			...
			async with ctx.typing():
				async with self.bot.ses.get("https://trace.moe/api/search?url=" + quote(image_url)) as tmoe_resp:
					try:
						mal_resp = (
							await
							AioJikan(session=self.bot.ses).anime((tmoe_resp := (await tmoe_resp.json())["docs"][0])["mal_id"])
						)
