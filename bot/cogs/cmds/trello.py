...
class Trello(Cog, name="Commands"):
	...
	async def add(self, ctx, *, title=None):
		...
		try:
			reaction, user = await self.bot.wait_for("reaction_add", timeout=15, check=lambda r, u: u == ctx.author and r.message == wiz and r.emoji in ('☑️', '➡️', '❌'))
		except TimeoutError:
		...
		if reaction.emoji == '☑️':
			...
			try:
				message = await self.bot.wait_for("message", timeout=60, check=lambda m: m.author == ctx.author and m.channel == ctx.channel)
			except TimeoutError:
				await wiz.edit(embed=Embed(title="Command timed out"), delete_after=7)
				return
		...
		await wiz.edit(embed=Embed(title=f"Do you want to attach any files to {bold(title)}?", description=f"Added description:\n{params['desc']}" if params.get("desc") else Embed.Empty))
		for r in ('☑️', '➡️', '❌'):
			await wiz.add_reaction(r)
		try:
			reaction, user = await self.bot.wait_for("reaction_add", timeout=15, check=lambda r, u: u == ctx.author and r.message == wiz and r.emoji in ('☑️', '➡️', '❌'))
