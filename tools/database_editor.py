...
class Options(NamedTuple):
    ...
    type: str = "0"
    rank: str | None = None
    single: Literal[True] | None = None
    awakened: Literal[True] | None = None
    field: str | None = None
    pool: str | None = None
    rate: str | None = None
    fragments: Literal[True] | None = None
    regex: Literal[True] | None = None


class DatabaseEditor:
    def __init__(self, database: dict[str, dict[str, dict[str, Any]]]):
        self.database = database
        self.logs_enabled = True

    # TODO: Make the argument parser more obvious, because
    # specifying the "names" argument here makes no sense.
    # python tools/database_editor.py listtables all
    def list_tables(self, options: Options) -> Literal[False]:
        """List the names of the tables available in the database."""
        for table_id in self.database:
            self._log(table_id)
        return False

    # database_editor.py --type <type> [--rank <rank>] additem "name 1" "name 2"
    # database_editor.py --type 2 --rank 3 additem "name 1" "name 2"
    def add_item(self, options: Options) -> bool:
        """Add a new item described by the specified options to the database."""
        if not options.type:
    ...
    def find_item(self, options: Options) -> Literal[False]:
    ...
    def delete_item(self, options: Options) -> bool:
        table = self.database.setdefault(TABLE_ITEMS, {})
        has_changed = False
        for name in options.names:
    ...
    def add_item_set(self, options: Options) -> bool:
        if len(options.names) != 3:
            raise ValueError("You must specify a valid itemset: valkyrie, weapon, stigmata.")

        def add_item(name: str, item_type: str, item_rank: str | None = None) -> None:
    ...
    def remove_pool(self, options: Options) -> bool:
        table = self.database.setdefault(TABLE_POOLS, {})
        has_changed = False
        for name in options.names:
    ...
    def add_pool_item(self, options: Options) -> bool:
        ...
        loot_table: list[dict[str, Any]] = pool.setdefault("loot_table", [])
    ...
    def remove_pool_item(self, options: Options) -> bool:
        table = self.database.setdefault(TABLE_POOLS, {})
        pool_id = self._find_id_by_field(TABLE_POOLS, "code", options.pool)
        pool = table.get(pool_id)
        if pool is None:
        ...
        for name in options.names:
            ...
            is_found = False
            for descriptor_index, descriptor in enumerate(loot_table):
                item_list: list[str] = descriptor.get("items", [])
                if item_id in item_list:
    ...
    def replace_pool_item(self, options: Options) -> bool:
    ...
    def show_pool(self, options: Options) -> bool:
        pools = self.database.setdefault(TABLE_POOLS, {})
        pool_id = self._find_id_by_field(TABLE_POOLS, "code", options.names[0])
        pool: dict[str, list[dict[str, Any]]] = pools.get(pool_id)
        if pool is None:
    ...
    def toggle_pool(self, options: Options) -> bool:
        pools = self.database.setdefault(TABLE_POOLS, {})
        pool_id = self._find_id_by_field(TABLE_POOLS, "code", options.names[0])
        pool = pools.get(pool_id)
        if pool is None:
    ...
    def _get_next_id(self, table_name: str) -> int:
        ...
        if table is None:
            return 1
        return max((int(key) for key in table), default=0) + 1

    def _get_pool_total_rate(self, pool_code: str) -> float:
        """Calculate the total drop rate for the items in the loot table of the pool identified by
        the specified ``pool_code``.
    ...
    def _find_ids_by_field(
        ...
        """Find all the identifiers in the table with the specified ``table_name`` whose fields
        identified by the specified ``field_name`` match the specified ``field_value``.
    ...
    def _find_valkyrie_fragment_id(self, valkyrie_name: str) -> str:
        """Find the unique identifier of fragment/soul that belongs to the Valkyrie with the
        specified name.
        """
    ...
    def _replace_pool_item(self, loot_table: dict, old_item_name: str, new_item_name: str) -> bool:
    ...
    def _replace_pool_valkyrie_fragment(
        ...
        if new_item_id is None:
            self._log(
                f"The fragment for item '{new_valkyrie_name}' doesn't "
                "exist, hence it won't replace any other fragments."
            )
            return False
        if self._replace_pool_item_internal(loot_table, old_item_id, new_item_id):
            self._log(
                f"The fragment of valkyrie '{old_valkyrie_name}' has been "
                f"replaced by the fragment of valkyrie '{new_valkyrie_name}'."
            )
            return True
        self._log(
    ...
    def _replace_pool_item_internal(
        self, loot_table: dict[dict[str, list[str]], Any], old_item_id: str, new_item_id: str
    ) -> bool:
        for descriptor in loot_table:
...
class Main:
    def __init__(self, database_file_path: str):
        ...
        with Path(database_file_path).open() as database_file:
            self._editor = DatabaseEditor(json.load(database_file))
        self._operations = {
            # generic
    ...
    def execute(self, options: Options) -> None:
        operation = self._operations.get(options.operation)
        if operation is not None:
            logging.info("Invoking operation '%s'...", options.operation)
            if operation(options):
                self._save_database()
            logging.info("Operation '%s' finished.", options.operation)
        else:
            logging.error("Invalid operation '%s'.", options.operation)

    def _save_database(self) -> None:
        """Save the database associated to the editor instance."""
        with Path(self._database_file_path).open("w+") as database_file:
            json.dump(self._editor.database, database_file, indent="\t")
