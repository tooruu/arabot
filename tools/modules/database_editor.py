...
class DatabaseEditor:
	...
	def add_pool_item(self, options) -> bool:
		...
		for _, name in enumerate(options.names):
			...
			if item_id in item_list:
				self._log(f"Item '{name}' is already added to the pool with the same rate, hence it won't be added again.")
				continue
	...
	def _add_item_internal(self, item_name: str, item_type: str, item_rank: str = None, is_single_stigmata: bool = False) -> str:
		table = self._get_or_initialize_value(self.database, TABLE_ITEMS, {})
	...
	def _replace_pool_valkyrie_fragment(self, loot_table, old_valkyrie_name: str, new_valkyrie_name: str) -> bool:
		...
		if new_item_id is None:
			self._log(f"The fragment for item '{new_valkyrie_name}' doesn't exist, hence it won't replace any other fragments.")
			return False
		if self._replace_pool_item_internal(loot_table, old_item_id, new_item_id):
			self._log(f"The fragment of valkyrie '{old_valkyrie_name}' has been replaced by the fragment of valkyrie '{new_valkyrie_name}'.")
			return True
		self._log(f"The fragment of valkyrie '{old_valkyrie_name}' cannot be found in the pool, hence it won't be replaced.")
		return False
