...
class Userinfo(Cog, category=Category.GENERAL):
    ...
    async def avatar(self, ctx: Context, *, member: AnyMember = False):
        ...
        avatars = (
            ...
            .set_footer(text=ctx._("{}'s global avatar").format(member.display_name)),
            disnake.Embed()
            .set_image(url=member.display_avatar.compat)
            .set_footer(text=ctx._("{}'s server avatar").format(member.display_name)),
        )
    ...
    async def banner(self, ctx: Context, *, member: AnyMember = False):
        ...
        if not banner:
            await ctx.send_("User has no banner")
            return
        await ctx.send(
            embed=disnake.Embed()
            .set_image(url=banner.maxres.compat)
            .set_footer(text=ctx._("{}'s banner").format(member.display_name))
        )
    ...
    async def userinfo(self, ctx: Context, *, member: AnyMemberOrUser = False):
        ...
        embed = (
            ...
            .add_field(ctx._("Created on"), format_dt(member.created_at, "D"))
        )
        description = defaultdict(list)
        if member.bot:
            description[0].append(ctx._("Bot"))
        if member.public_flags.spammer:
            description[0].append(f"**{ctx._('Marked as spammer')}**")
        ...
        if isinstance(member, disnake.Member):
            ...
            if member.guild_avatar:
                description[1].append(f"[{ctx._('Server avatar')}]({member.guild_avatar})")
            if member.pending:
                description[0].append(ctx._("Pending verification"))

            if member.joined_at:
                embed.add_field(ctx._("Joined on"), format_dt(member.joined_at, "D"))
            if member.nick:
                embed.add_field(ctx._("Nickname"), member.nick)
            if member.activity:
                embed.add_field(ctx._("Activity"), member.activity.name)
            if member.premium_since:
                embed.add_field(ctx._("Boosting since"), format_dt(member.premium_since, "R"))
            if member.current_timeout:
                embed.add_field(ctx._("Muted until"), format_dt(member.current_timeout, "D"))
            elif member.voice and member.voice.channel:
                embed.add_field(ctx._("Talking in"), member.voice.channel.mention)
            embed.add_field(ctx._("Highest non-dummy role"), member.top_perm_role.mention)
